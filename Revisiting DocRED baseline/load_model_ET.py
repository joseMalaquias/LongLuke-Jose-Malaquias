...
from configurations.my_configuration import LukeConfig_LongDOCRED_ET
...
class myLukeSelfAttention(nn.Module):
    ...
    def forward(
        ...
        del output_word_hidden_states, output_entity_hidden_states, attention_probs, context_layer
...
class ModifiedLuke(LukeForEntityClassification):
    def __init__(self, config):
        ...
        self.classifier = torch.nn.Linear(in_features = 1024, out_features = 7, bias = True)
...
def create_new_model(save_model_to, attention_window, max_pos):
    ...
    model.config = LukeConfig_LongDOCRED_ET()
    model.luke.config = LukeConfig_LongDOCRED_ET()
    num_labels = 7
...
model_path = f'{training_args.output_dir}/LongLuke_DOCRED_ET_before_finetune'
model_path_finetuned = "/mnt/shared/home/jose.luis.malaquias.ext/reDOCRED/ET_for_reDOCRED/model_finetuned_ET_reDocRED_3_mais_2"

if not os.path.exists(model_path):
    os.makedirs(model_path)

logger.info('LOADING BASELINE LUKE MODEL NOT FINETUNED!!!')

model = LukeForEntityClassification.from_pretrained("studio-ousia/luke-large")
tokenizer = LukeTokenizer.from_pretrained("studio-ousia/luke-large")
#model = ModifiedLuke.from_pretrained("/mnt/shared/home/jose.luis.malaquias.ext/reDOCRED/baseline_reDOCRED_ET/model_finetuned_baseline_new")
