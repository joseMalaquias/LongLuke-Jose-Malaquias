...
from transformers import LukeModel, LukeTokenizer, Trainer, TrainingArguments, TextDataset, LukeForEntityPairClassification,\
    HfArgumentParser
from transformers import LukeForEntityClassification
from configurations.my_configuration import LukeConfig_LongDOCRED
...
class myLukeSelfAttention(nn.Module):
    ...
    def forward(
        ...
        del output_word_hidden_states, output_entity_hidden_states, attention_probs, float_mask, diagonal_mask
        del w2w_query_layer, w2e_query_layer, e2w_query_layer, e2e_query_layer, w2w_key_layer, w2e_key_layer, e2w_key_layer, e2e_key_layer
        return outputs
    ...
    def _pad_by_window_overlap_except_last_row(chunked_hidden_states, batch_size, num_heads, seq_len):
        ...
        chunked_hidden_states[:, 0, :, :] = F.pad(chunked_hidden_states[:, 0, :, window_overlap:],
                                                  pad=(0, window_overlap), value=0)
        chunked_hidden_states = chunked_hidden_states.view(batch_size, num_heads, seq_len,
                                                           seq_len)  # batch x num_heads x seq_len x seq_len
...
class ModifiedLuke(LukeForEntityPairClassification):
...
def create_new_model(save_model_to, attention_window, max_pos):
    #model_old = LukeForEntityPairClassification.from_pretrained("studio-ousia/luke-large-finetuned-tacred")
    model = LukeForEntityPairClassification.from_pretrained("studio-ousia/luke-large")
    tokenizer = LukeTokenizer.from_pretrained("studio-ousia/luke-large-finetuned-tacred")
    model.config = LukeConfig_LongDOCRED()
    model.luke.config = LukeConfig_LongDOCRED()
    num_labels = 97
...
model_path = f'{training_args.output_dir}/LongLuke_Re_DOCRED_not_finetuned'
#model_path_finetuned = "/Users/jose.luis.malaquias.ext/Documents/GitTese/luke_87050/Agosto_DOCRED/DOCRED_finetuned"
#model_path_competition = "/mnt/shared/home/jose.luis.malaquias.ext/novembro/model_finetuned_5epocs"
model_path_evaluation = "/mnt/shared/home/jose.luis.malaquias.ext/reDOCRED/Revisiting_DOCRED/model_finetuned_balanced_50_negatives"
...
#model.to(device) DONT FORGET TO ADD THIS TO CODE

model = ModifiedLuke.from_pretrained(model_path_evaluation)
